...
*/
KNET_STATIC bool KnetIsCurrentMainThread(void)
{
...
struct rte_mbuf *KNET_PktAlloc(uint32_t poolId)
{
    ...
    if ((batch->processMode != 0) && KnetIsCurrentMainThread()) {
        ...
        if (ret != 0) {
            KNET_ERR("Pkt alloc bulk without cache failed, ret %d, poolId %u, batch num %u", ret, poolId,
                MBUF_BATCH_NUM);
            goto err;
        }
    } else {
        ...
        if (ret < 0) {
            KNET_ERR("Mbuf alloc bulk failed, ret %d, poolId %u, batch num %u", ret, poolId, MBUF_BATCH_NUM);
            goto err;
        }
    }
 ...
 *            若不为1就释放会导致后续申请的时候dpdk alloc接口的mbuf ref == 1的check失败，目前tcp协议栈调用mbuf free接口时mbuf ref一定为1
...
void KNET_MbufAttachExtBuf(struct rte_mbuf *mbuf, void *addr, uint64_t iova, uint16_t len, void *shinfo)
{
    if (KNET_GetCfg(CONF_COMMON_COTHREAD)->intValue == 1) {
        ((struct rte_mbuf_ext_shared_info *)shinfo)->refcnt++;
    } else {
        rte_mbuf_ext_refcnt_update((struct rte_mbuf_ext_shared_info *)shinfo, 1);
    }
    rte_pktmbuf_attach_extbuf(mbuf, addr, (rte_iova_t)iova, len, (struct rte_mbuf_ext_shared_info *)shinfo);
}
