...
KNET_STATIC int KnetAdapterBondAdd(uint16_t bondedPortId, uint16_t slavePortId)
{
    int ret = 0;
...
KNET_STATIC int KnetAdapterBondSlavesGet(uint16_t bondPortId, uint16_t slaves[], uint16_t len)
{
    int ret = 0;
    #if RTE_VERSION >= RTE_VERSION_NUM(23,11,0,0)
...
KNET_STATIC int AddSlavesToBond(int bondPortID, uint16_t slavePortIds[], uint16_t slavePortNum)
{
    ...
    for (uint16_t i = 0; i < slavePortNum && i < KNET_BOND_SLAVE_NUM; i++) {
        ret = KnetAdapterBondAdd(bondPortID, slavePortIds[i]);
        if (ret != 0) {
...
int KNET_BondWaitSlavesReady(int bondPortID)
{
    ...
    while (waitCounter > 0) {
        ...
        if (KnetAdapterBondSlavesGet(bondPortID, activeSlaves, KNET_BOND_SLAVE_NUM) == KNET_BOND_SLAVE_NUM) {
            break;
        }
        sleep(1);  // 睡眠等待 1s
...
int KNET_BondUninit(int procType)
{
    ...
    for (uint16_t i = 0; i < KNET_BOND_SLAVE_NUM; i++) {
        ...
        if (ret != 0) {
            KNET_ERR("K-NET uninit dpdk port %d failed, ret %d", i, ret);
            flag = 1;
        }
    }
