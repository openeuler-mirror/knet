...
typedef struct {
    ...
    HashTblCb infoCbs[DEFAULT_HASH_TBL_NUM];      /* 表信息结构体起始地址，代替以前的全局遍历方式 ，
...
{
    /* 获取TBM全局控制数据指针 */
    HashTblCb *tblInfo = g_tblMng.infoCbs;
...
{
    HashTblCb *tblInfo = NULL;

    /* 获取TBM全局控制数据指针 */
    tblInfo = g_tblMng.infoCbs + tableId;

    (void) memset_s((void *) tblInfo, sizeof(HashTblCb), 0, sizeof(HashTblCb));
}
...
void HashTableReleaseAllEntry(HashTblCb *table)
{
    uint8_t *key = NULL;
    uint8_t *entry = NULL;
    uint32_t iter = 0;
    ...
    while (KNET_HalAtomicRead64(&table->usedNum) > 0) {
        ...
        hash_sig_t hash_value = g_hashFunc((uint8_t *)key, table->keySize);  // 重新计算hash值
...
int KNET_CreateHashTbl(KNET_HashTblCfg *cfg, uint32_t *tableId)
{
    ...
    int32_t ret = ParameterCheck(cfg, tableId);
    if (ret != 0) {
    ...
    HashTblCb *tblInfo = g_tblMng.infoCbs + localTableId;
...
int KNET_HashTblAddEntry(uint32_t tableId, const uint8_t *key, const uint8_t *data)
{
    ...
    uint8_t *entry = (uint8_t *)malloc(tblInfo->entrySize);
    if (entry == NULL) {
...
int KNET_HashTblDelEntry(uint32_t tableId, const uint8_t *key)
{
    ...
    int32_t delPos = rte_hash_del_key_with_hash(tblInfo->handle, key, hash_value);
    if (delPos < 0) {
...
int KNET_GetHashTblFirstEntry(uint32_t tableId, uint8_t *key, uint8_t *data)
{
    ...
    uint32_t iter = 0;
    uint8_t *entry = NULL;
    int32_t ret = rte_hash_iterate(tblInfo->handle, (const void **)&key, (void **)&entry, &iter);
    if (ret < 0) {
...
int KNET_GetHashTblNextEntry(uint32_t tableId, const uint8_t *curKey, uint8_t *key, uint8_t *data)
{
    ...
    hash_sig_t hash_value = g_hashFunc((uint8_t *)curKey, tblInfo->keySize);  // 重新计算hash值
    int32_t pos = rte_hash_lookup_with_hash(tblInfo->handle, curKey, hash_value);
    if (pos < 0) {
...
void KNET_HashTblDeinit(void)
{
    ...
    if (g_tblMng.tableIdNum == 0) {
        KNET_ERR("Tbm deinitialize but not initialized");
        KNET_SpinlockUnlock(&g_hashTableLock);
        return;
    }
