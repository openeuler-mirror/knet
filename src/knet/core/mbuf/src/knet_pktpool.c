...
static uint32_t PktPoolCfgSanityCheck(const KnetPktPoolCfg *cfg)
{
...
void PktPoolObjInitCallback(struct rte_mempool *mp, void *para, void *obj, unsigned index)
{
    ...
    void *pkt = KnetMbuf2Pkt((struct rte_mbuf *)obj);
    KnetPktInit init = (KnetPktInit)para;
    if (init != NULL) {
        init(pkt);
    }
}

static struct rte_mempool *MemPoolCreate(const KnetPktPoolCfg *cfg)
{
    uint32_t privDataSize;
    uint32_t dataroomSize;

    privDataSize = (uint32_t)cfg->privDataSize + KNET_PKT_DBG_SIZE;
    ...
    dataroomSize = (uint32_t)cfg->dataroomSize + RTE_PKTMBUF_HEADROOM;
...
static uint32_t PktPoolCreate(uint32_t poolId, const KnetPktPoolCfg *cfg)
{
    struct rte_mempool *mp = NULL;
    KnetPktPoolCtrl *poolCtrl = &(g_knetPktModCtrl.poolCtrl[poolId]);

    mp = MemPoolCreate(cfg);
    if (mp == NULL) {
...
uint32_t PktPoolCreateInPrimary(const KnetPktPoolCfg *cfg, uint32_t *poolId)
{
    uint32_t ret;

    /* 编译阶段确认偏移值是否发生变化 */
    RTE_BUILD_BUG_ON(sizeof(struct MbufShadow) != sizeof(struct rte_mbuf));
    RTE_BUILD_BUG_ON(offsetof(struct MbufShadow, pktLen) != offsetof(struct rte_mbuf, pkt_len));
    RTE_BUILD_BUG_ON(offsetof(struct MbufShadow, txOffload) != offsetof(struct rte_mbuf, tx_offload));
    RTE_BUILD_BUG_ON(offsetof(struct MbufShadow, olFlags) != offsetof(struct rte_mbuf, ol_flags));
    RTE_BUILD_BUG_ON(offsetof(struct MbufShadow, privSize) != offsetof(struct rte_mbuf, priv_size));
...
uint32_t KNET_PktModInit(void)
{
    ...
    iRet = pthread_mutex_init(&g_knetPktModCtrl.mutex, NULL);
    if (iRet != 0) {
