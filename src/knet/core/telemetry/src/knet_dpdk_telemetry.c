...
static int32_t KnetDpdkRuntimeDirInit(void)
{
...
static bool KnetDpdkTelemetryJsonPidCheck(const char *buffer)
{
    /* 解析 JSON 数据 */
...
static bool KnetDpdkTelemetrySocketPidCheck(void)
{
    int sockfd = -1;
    ...
    if (!KnetDpdkTelemetryJsonPidCheck(buffer)) {
 ...
 */
static int32_t KnetDpdkTelemetryLinkCreate(void)
{
    int32_t ret;
    struct sockaddr_un addr = {0};
    int sockfd = -1;
    ...
    (void)memset_s(&addr, sizeof(addr), 0, sizeof(addr));
    addr.sun_family = AF_UNIX;
    ret = strncpy_s(addr.sun_path, sizeof(addr.sun_path), g_knetTelemetrySocketNew, strlen(g_knetTelemetrySocketNew));
    if (ret != 0) {
...
static int32_t KnetDpdkTelemetryFindSocket(void)
{
    struct stat fileStat = { 0 };
    if (lstat(g_knetTelemetrySocket, &fileStat) == -1 || S_ISSOCK(fileStat.st_mode) == 0) {
        KNET_ERR("K-NET dpdk telemetry file is not socket, %s", g_knetTelemetrySocket);
        return -1;
    }

    if (KnetDpdkTelemetrySocketPidCheck()) {
        return 0;
    }
...
int32_t KnetTelemetryMzInit(void)
{
    const struct rte_memzone *mz = NULL;
    mz = rte_memzone_reserve(KNET_TELEMETRY_MZ_NAME, sizeof(TelemetryInfo), SOCKET_ID_ANY, 0);
    if (mz == NULL) {
    ...
    (void)memset_s(telemetryInfo, sizeof(TelemetryInfo), 0, sizeof(TelemetryInfo));

    return KNET_OK;
}

int32_t KnetRegTelemetryCmd(void)
{
    /* 支持dp报文打点统计 */
    telemetry_cb telemetryCallbackFunc = NULL;

    if (KNET_GetCfg(CONF_COMMON_MODE).intValue == KNET_RUN_MODE_SINGLE) {
        telemetryCallbackFunc = KNET_TelemetryStatisticCallback;
    } else {
        telemetryCallbackFunc = KNET_TelemetryStatisticCallbackMp;
        if (KnetTelemetryMzInit() != KNET_OK) {
            /* 内部已打印日志 */
            return KNET_ERROR;
        }
    }

    int ret = rte_telemetry_register_cmd("/knet/ethstats", telemetryCallbackFunc, "Return dp statistic");
    if (ret < 0) {
        KNET_ERR("K-NET register telemetry cmd /knet/ethstats failed, ret %d", ret);
        return KNET_ERROR;
    }
