...
{
    if (g_baseFunc.memFns == NULL) {
        g_baseFunc.memFns = GetDefaultMemfns();
    }

    if (g_baseFunc.semFns == NULL) {
        g_baseFunc.semFns = GetDefaultSemFns();
    }

    if (g_baseFunc.timeFn == NULL) {
        g_baseFunc.timeFn = GetTimeFns();
    }
