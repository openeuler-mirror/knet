...
1.  配置交换机。
    ...
    4.  客户端也需要进行上述[步骤1.1](#dpdk-li1)到[步骤1.3](#dpdk-li2)的操作，将2个PF口加入到一个trunk口并进行查看端口配置确认是否成功加入。
...
4.  运行业务。
    ...
    1.  服务端用户态劫持启动iPerf3。
        ...
        > **说明：** 
        >-   taskset -c  _20-39_：将指定的进程绑定到CPU核心20\~39上运行，用户使用时参考[绑核与网卡所在NUMA一致](../reference/performance_tuning/bound_same_numa_nic.md)中的步骤1和步骤2确认绑定的CPU范围。
        >-   -s：以服务器模式运行。
        >-   -4：仅使用IPv4协议。
        >-   -p：指定服务器端口。
