...
1.  业务适配K-NET共线程模式。
    ...
    -   总结
        ...
        上述伪代码的核心在于：引入knet\_socket\_api.h头文件，在业务进程初始化中进行**knet\_init**\(\)，设置线程CPU亲和性，注意需与"ctrl\_vcpu\_ids"不同；在业务线程运行时，首先进行**knet\_worker\_init\(\)**初始化worker，并可以通过**knet\_is\_worker\_thread\(\)**判断当前线程是否在用户态协议栈线程中；其次需要保证创建的socket fd均为非阻塞状态；再次需要保证**knet\_worker\_run\(\)**一直被调用。
