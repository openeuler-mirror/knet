...
|optname|选项名称|<li>SO_REUSEADDR：地址重用，0表示关闭；非0表示开启，默认关闭。<li>SO_REUSEPORT：端口重用，0表示关闭；非0表示开启，默认关闭。<li>SO_KEEPALIVE：保活选项，0表示关闭；非0表示开启，默认关闭。<li>SO_LINGER：linger选项，默认关闭。<li>SO_SNDBUF：获取发送缓存即高水位。<li>SO_RCVBUF：获取接收缓存即高水位。<li>SO_SNDTIMEO：获取阻塞模式发送超时时间 。<li>SO_RCVTIMEO：获取阻塞模式接收等待超时时间 。<li>SO_ERROR：获取socket的错误信息，读取后内部清零。<li>SO_PROTOCOL：获取套接字协议类型。<li>TCP_NODELAY：获取是否禁止TCP的Nagle算法，默认开启Nagle算法。<li>TCP_KEEPIDLE：获取指定的空闲时间后启动保活探测的时间，单位秒。<li>TCP_KEEPINTVL：获取保活探测的时间间隔，单位秒。<li>TCP_KEEPCNT：获取保活探测的次数。<li>TCP_CORK：获取cork选项。1表示开启，0表示关闭，默认关闭。<li>TCP_DEFER_ACCEPT：获取延迟建链的超时时间，单位秒。<li>TCP_MAXSEG：获取设置的TCP最大报文段。<li>TCP_INFO：获取TCP信息，只能获取部分信息。<li>IP_TOS：获取设置的TOS。|
|*optval|选项值|非空指针。|
