...
**accept4(int sockfd, struct sockaddr \*addr, socklen\_t \*addrlen, int flags\)**

## 接口描述

在侦听socket上接收新的连接，并设置socketFd属性。
...
|*addr|通信对端地址|可以为空指针。会填写为对端socket的地址，长度根据协议地址类型而定，参考DP_Socket接口支持的协议。|
|*addrlen|地址长度|可以为空指针。必须设置为可以包含地址结构体的长度，函数返回时设置为实际的长度。|
|flags|flags标志位|处理标志位支持0，SOCK_NONBLOCK，SOCK_CLOEXEC（默认使能）。|
