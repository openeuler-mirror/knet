...
|ctrl_vcpu_ids     |控制面绑核，主要将控制面绑定的核号固定。如果需要绑多个核，可以设置为： `"ctrl_vcpu_ids": [`<p>`0,1,2,3`</p>`],`<p> **说明：** </p> <li>控制面绑核核号小于最大vCPU个数。如果vCPU个数小于最大值：319，那么最大值取最大vCPU个数-1。</li><li>物理机场景：vCPU个数为实际CPU个数。</li><li>虚拟机场景：vCPU个数为配置虚拟机的个数。</li>|[0]|0~319|
...
|bdf_nums|需要DPDK驱动接管的接口BDF号。需要配置为非空，如果为空则返回错误。<p> **说明：** </p> <p>bond_enable为0时，此配置项仅填写1个BDF号；bond_enable为1时，多个网口需按如下形式填写BDF号。</p><p> `"bdf_nums": [` </p><p> `"0000:06:00.0",` `"0000:06:00.1"`</p><p>`],`</p>|["0000:06:00.0"]|非空|
...
|max_epollcb  |最大epoll实例数量。|64|1~1000000|
