# 参考

-   [性能调优](performance_tuning/tuning_menu.md)
    -   [绑核与网卡所在NUMA一致](performance_tuning/bound_same_numa_nic.md)
    -   [网卡队列调优](performance_tuning/nic_queue_tuning.md)
    -   [BIOS调优](performance_tuning/bios_tuning.md)

-   [脚本参考](script_reference/script_menu.md)
    -   [knet\_ctl.sh（运维脚本）](script_reference/knet_ctl.md)
    -   [dpdk-telemetry.py网卡统计信息获取脚本](script_reference/dpdk-telemetry.md)

-   [故障处理](troubleshooting/troubleshooting_list.md)
    -   [Bond故障](troubleshooting/Bond.md)
    -   [安装故障](troubleshooting/installation.md)
    -   [内核流量转发故障](troubleshooting/kernel_traffic_forwarding.md)
    -   [时延故障](troubleshooting/latency.md)
    -   [内存故障](troubleshooting/memory.md)
    -   [多进程故障](troubleshooting/multi-process_mode.md)
    -   [网络故障](troubleshooting/network.md)
    -   [收集运维信息故障](troubleshooting/OM.md)
    -   [性能波动故障](troubleshooting/performance.md)
    -   [telemetry故障](troubleshooting/telemetry.md)
    -   [流分叉故障](troubleshooting/flow_bifurcation.md)
    -   [卸载故障](troubleshooting/uninstallation.md)
    -   [重启虚拟机失败进入shell界面](troubleshooting/virtua_machine.md)
    -   [零拷贝故障](troubleshooting/zero_copy.md)
    
-   [FAQs](FAQs/batch_om.md)