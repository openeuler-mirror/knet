...
Data Acceleration Kit K-NET（K-Network，网络加速套件）是一款网络加速套件，提供统一的软件框架，发挥软硬协同优势，释放网卡硬件性能。
...
- 软硬件要求满足[环境要求](./release_note.md#版本配套关系)，本文以openEuler 22.03 LTS SP4为例，客户端与服务端需保持一致。
- 已在服务端和客户端安装SP670驱动和固件。安装方式请参见[《SP220&SP600 标准网卡 用户指南》](https://support.huawei.com/enterprise/zh/doc/EDOC1100309168/a7897085?idPath=23710424|251364417|9856629|253287505)中“软件安装”章节。网卡模板为“0”或者“3”。可执行以下命令检查：
    ...
    ![网卡配置模板](./figures/zh-cn_image_0000002487030394.png)
    ...
    切换后执行**reboot**重启。
- 服务端glibc版本为2.10以上，2.10以上版本会开启堆栈保护。
    可通过以下命令查看版本：
...
- 安装基础依赖。
    ...
    ```bash
    yum install -y git gcc tar gzip vim jq rpm-build python python3-devel cmake
    ```
...
1. 安装DPDK 21.11.7。
    1. 安装编译依赖。
        ...
        ```bash
        yum install -y libatomic meson ninja-build python3-pyelftools libibverbs numactl numactl-devel zlib-devel
        ```
    ...
    4. 安装DPDK驱动。
        ...
        ![meson构建结果](./figures/zh-cn_image_0000002503958012.png)
        ...
        ![ninja构建结果](./figures/zh-cn_image_0000002535517975.png)
        ...
        ![ninja安装结果](./figures/zh-cn_image_0000002503798182.png)
...
2. 安装dpdk-hinic3驱动。
    ...
    2. 获取配套版本的commitid。
        ...
        配套的dpdk-hinic3版本请参考[版本说明书](./release_note.md#产品版本信息)，单击需要的版本跳转查看对应的commitid。
        commitid位置示意如下：
...
1. 安装系统依赖。
    ...
    ```bash
    yum install -y libcap-devel libboundscheck
    ```

2. 下载K-NET源码包，以安装路径“/home/knet-repo”为例。
...
2. 查看网卡信息。
    ...
    2. 查看网卡IP地址和MAC地址。
        ...
        根据网口enp4s0f0获取的IP地址：192.168.1.58，MAC地址：ac:dc:ca:xx:xx:xx。
...
3. 配置大页内存。
    请在服务端配置大页内存，具体node编号根据查询到的网卡所在NUMA进行更改，大页数量与单个大页大小根据实际情况配置，系统可用单页大小可使用tab补全或在/sys/devices/system/node/node0/hugepages路径下查看。此处为node 0分配2个大小为1048576kB（1GB）的大页。
...
1. 测试网络连通性。在客户端ping服务端IP地址，确保网络是连通的，此处192.168.1.58为前述查询所得服务端地址。
...
4. 测试完成后在服务端结束K-NET进程，按Ctrl+C结束Redis进程。
...
5. 测试完成后在服务端结束K-NET进程，按Ctrl+C结束Redis进程。
...
关于K-NET多进程加速、零拷贝和共线程等更多特性详细使用方式请参见[特性使用](./feature_guide/feature_menu.md)。
