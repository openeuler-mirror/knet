# Release Note

## 产品版本信息

|  产品名称      |   软件名称   | 软件版本 |
|------------|-------------|-------------|
| Data Acceleration Kit | 网络加速套件（K-NET）   | [26.1.RC1](https://gitcode.com/openeuler/knet/tags/knet-26.1.rc1-0630)  |

## 版本配套关系

### 鲲鹏环境

|  项目       |    兼容性1  |    兼容性2 |   兼容性3 | 兼容性4 |
|------------|-------------|-----------|-----------|---------|
| 操作系统  | openEuler 22.03 LTS SP4  | CTyunos-2.0.1-220311-aarch64(4.19.90-2102.2.0.0062.ctl2.aarch64) | openEuler 24.03 LTS SP3 | Kylin-Sever-V10-SP3-2403-Release |
| 服务器  |  TaiShan 200服务器（型号2280 VD）  | TaiShan 200服务器（型号2280 VD）    |  TaiShan 200服务器（新型号）   |  TaiShan 200服务器（型号2280 VD） |
| 网卡  | SP670  | SP670 | SP670 | SP233 |
| CPU  | 鲲鹏920 7260处理器  | 鲲鹏920 7260处理器    | 鲲鹏920 7260T处理器  | 鲲鹏920新型号 7260T处理器 |
| DPDK  | 21.11.7  | 21.11.7  | 23.11.6  | 21.11.7 |

### x86环境

| 项目       | 兼容性  |
|------------|-------|
| 操作系统  | openEuler 22.03 LTS SP1   |
| 服务器  |  FusionServer 2288H V6  |
| 网卡  | SP670  |
| CPU  | 1/2个第三代英特尔至强可扩展处理器（Ice Lake）（8300/6300/5300/4300系列）  |
| DPDK  | 21.11.7  |

> [!NOTE]说明
> 以上配套环境需保持服务端与客户端一致。
> 适配场景请查看[特性支持](./feature_guide/feature_overview.md)。
> 目前仅兼容性3配套环境支持DPDK23.11.6版本，仅支持DPDK基础功能，不包括流量分叉、Bond卸载等网卡扩展特性。

### 软件配套关系

| 项目       | 版本  |获取地址|
|------------|-------|-------|
| Redis  | 6.0.20   |[获取链接](https://github.com/redis/redis/tree/6.0.20)|
| DPDK  |  21.11.7/23.11.6 |[获取链接](https://fast.dpdk.org/rel)|
| iPerf3  | 3.16  |[获取链接](https://github.com/esnet/iperf/releases/tag/3.16)|
| SockPerf  | 3.10  |[获取链接](https://github.com/Mellanox/sockperf/archive/3c65ad99cd385e18f8a2a655c19826e81a4d17e8.zip)|
| TPerf  | 1.0  |[获取链接](https://github.com/bytedance/libtpa/archive/3c9f05df7b7c8ebc46bfebc83c316ec50f149e1c.zip)|
| dpdk-hinic3 | hinic3-26.1.rc1-0630|[获取链接](https://atomgit.com/openeuler/dpdk/tags/hinic3-26.1.rc1-0630.r1) |

## K-NET 1.4.0

### 更新说明

- 新增Kylin-Server-V10-SP3-2403-Release版本与SP233网卡的支持。
- 支持Socket同时被epoll/poll/select侦听。

### 已解决问题

无

### 遗留问题

无

## K-NET 1.2.0

### 更新说明

- 新增DPDK 23.11.6版本的支持。

### 已解决问题

无

### 遗留问题

无

## K-NET 1.0.0

### 更新说明

<term>K-NET</term>网络加速套件首次发布在开源社区。

K-NET作为网络协议加速框架，北向提供统一的Socket API，南向提供统一的数据IO抽象层，同时提供配置文件，允许用户通过配置进行协议路由。K-NET加速框架通过规范协议适配接口，同时内部集成了不同的协议栈，当前已经集成了基于以太的用户态TCP/IP协议，未来会继续集成基于RDMA的用户态协议和基于UB的协议栈，来满足和适配大数据、数据库、分布式存储等多个业务场景。

- K-NET框架Framework
    - Socket透明替换：提供Socket透明替换接口支撑业务零侵入修改。
    - 提供插件化框架：插件化框架可以基于配置适配不同的协议，如用户态TCP协议栈、RDMA和UB协议等系统。
    - 提供协议编排能力：协议编排主要提供基础的协议进行编排，业务在启动时加载协议编排的能力使能业务处理。
    - 配置管理：加载协议栈和资源配置信息。

- 协议插件层
    - 提供一个用户态TCP/IP协议栈：用户态TCP/IP高性能协议栈，免除内核态和用户态的数据拷贝和系统调用，实现数据面高性能加速。

## 修订记录

|发布版本| 发布日期   | 修订说明  |
|------|------|-------|
|  01    | 2026-09-30  | 第一次正式发布。|
