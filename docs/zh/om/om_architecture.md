...
- 抓包工具**dumpcap**：为K-NET提供调试抓包功能，提供详细的网络实时监测细节，有两种适配K-NET的抓包模式。
    - 单进程抓包，基于主进程间的socket共享主进程收发队列，将共享报文写入文件，文件可由tcpdump命令和Wireshark工具查看。定位工具在使用抓包获取前需要先编译生成抓包程序。K-NET绑定的抓包获取能力在其他场景抓包可能失败，因此该功能仅用于K-NET。
    - 多进程抓包，对多个K-NET加速进程提供抓包能力，可用过滤条件筛选目标进程的隔离数据包。多进程抓包时，仅能抓到在抓包工具启动之前已经启动的业务进程的数据包，若业务进程在抓包工具启动后才启动，则无法抓取其数据包，此时需同时重启抓包工具和业务进程，或确保业务进程先于抓包工具启动。
