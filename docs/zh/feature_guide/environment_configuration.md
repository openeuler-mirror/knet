...
4. 在Linux行输入以下配置。若无需永久默认配置可跳过此步骤，跳转至[步骤8](#确认所用网口)。
    ...
    > [!NOTE]说明
    >- 此处设置默认大页内存的大小为1GB，数量为2。大页内存大小请根据[步骤2](#配置物理机内存大页)中查询到的主机支持的大页类型更改。
...
2. **标准模式**：DPDK接管网卡。（流量分叉模式请跳过此步骤）
    ...
    3. DPDK接管网卡。<a id="DPDK接管网卡"></a>
        ...
        > [!NOTICE]须知
        > 接管网卡后，内核网络协议栈将无法直接使用该网卡进行通信，必须启动K-NET进程，待K-NET初始化完成后才能开展K-NET加速业务通信。对于需要同时使用K-NET和内核协议栈的场景。可参考[网卡流量分叉功能](./traffic_bifurcation.md)或[内核流量转储功能](./kernel_traffic_forwarding.md)进行部署。
...
3. 配置K-NET动态库、knet_mp_daemon、knet_comm.conf以及业务软件相关权限。
    ...
    >- 此处以Redis作为示例。
    >- 若为root用户可跳过此步骤。
    >- 以下setcap命令对普通用户进行提权操作，可能会有安全问题。对于提权可能的风险，建议用户通过其他手段防护，如权限最小化，只能被指定的运行用户使用。
...
4. 设置“XDG_RUNTIME_DIR”启动环境变量，普通用户未设置该变量会产生错误，root用户请跳过此步骤。
     ...
     > [!NOTE]说明
     > 用户名使用KNET_USER作为占位符进行示例，运行时请将其替换为实际用户名。环境变量路径涉及的权限及安全需要用户保证。
