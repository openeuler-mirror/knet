...
> - 当K-NET进程connect目标IP地址为`0.0.0.0`、`127.0.0.1`或配置文件中的本机IP地址时，连接自动转为内核fd，通过内核协议栈完成本地通信。
...
K-NET进程作为TCP服务端侦听端口时，同一台服务器上的其他进程可以直接通过该端口与K-NET进程建立TCP连接，无需额外配置。

1. K-NET进程启动iPerf3服务端。

   服务端IP地址以192.168.1.6为例；具体需要替换为网卡配置的IP地址，且与K-NET配置文件中IP地址保持一致。
...
### 示例二：K-NET服务端绑定`0.0.0.0`，同机客户端连接

K-NET进程bind `0.0.0.0`时，内核侧侦听所有本地IP地址，同一台服务器上的其他进程可直接通过配置IP地址连接。
